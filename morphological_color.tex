...
%\ead[url]{www.disi.unitn.it/~boato/}
...
In this paper we present a non trivial extension of a recent work \cite{de2017detecting} which deterministically detect erosion and dilation of binary images. The proposed extension works on grayscale images by detecting morphological filters application in an accurate way both in uncompressed and compressed images. The method also allows for erosion versus dilation discrimination and in many cases also for the adopted structural element identification. %both useful in image phylogeny reconstruction aiming at reconstructing the transformations between near-duplicates as modified and original images can be considered in our case. 
...
%INSERIRE UN BLOCCO DENOMINATO "BLOCK SELECTION" PRIMA DELLO SDOPPIAMENTO NELLO SCHEMA E METTERE UN UNICO BLOCCO SVM ALLA FINE.
...
%These data were extracted for each quality factor and used to trained 2 classifiers: eroded vs unfiltered, and dilated vs. unfilterd. To this porpose, for each dataset we randomly selected a subset of images to train the classifier using two different percentages (70\% and 80\%). Each experiment was repeated 5 times and the results were averaged over the different trials.

\section{Experimental Results and Discussion}

\begin{comment}
\subsection{Scenario 1: Detection on compressed images}
\subsection{Scenario 2: Detection after noise attacks}
\subsection{Scenario 3: Confusion with other filters}

\section{PEZZI PRECEDENTI}


\subsection{Eroded vs. Unprocessed Input}

Given a compressed image $E$, previously eroded with structural element $B$, and a second compressed image $I$ with no previous morphological filtering of any nature, let us consider the erosion pipeline tuned with structural element $B$. The opening procedure results in the following:

\begin{itemize}
\item  $E^* = E \circ B \rightarrow E_{diff} = |E - E^*|$
\item  $I^* = I \circ B \rightarrow I_{diff} = |I - I^*|$
\end{itemize}

Histogram information is extracted from both $E_{diff}$ and $I_{diff}$; the result is represented in Figure \ref{fig:hist_ero}.

The eroded image exhibits great similarity with its opened version, with $E_{diff}$ characterized by grey-levels close to zero. As a consequence, the corresponding histogram will be restricted a few bins, falling to zero very quickly. At the same time, applying the same detector configuration to the compressed unprocessed image produces a larger difference, hence the histogram of $I_{diff}$ covers a much wider range of bins. 

The compression rate is critical: greater compression results in the presence of more artifacts, hence reducing the steepness of a correct configuration between input and detector. Figure \ref{fig:hist_ero_80} shows the same processing as the previous example, although considering $QF = 80$. In this scenario, the histogram for $E_{diff}$ still tends to zero much faster than that of $I_{diff}$ , however its steepness is reduced.


\subsubsection{Eroded vs. Dilated Input}

Let us now consider the same compressed eroded image $E$ and a compressed image $D$ previously dilated with structural element $B^* \neq B$. The detector is again tuned with structural element $B$.

The result of such configuration, pictured in Figure \ref{fig:hist_dil_ero_diff}, is analogous to the scenario with unprocessed input. When both the investigated morphological operation and the structural element of the detector both match the type of processing applied to the input, then the absolute difference image exhibits few gray-levels. The same consideration is valid also in case of matching structuring element $B$ for both dilated and eroded compressed images (see Figure \ref{fig:hist_dil_ero_same}).


\subsubsection{Eroded vs. Eroded Input}

Let us consider this last scenario, involving two compressed images $E$ and $E^*$, previously eroded with two different structural elements $B$ and $B^*$, with $B^*$ completely unrelated to $B$. Again, the embedded detector is tuned with kernel $B$. Resulting histograms are depicted in Figure \ref{fig:hist_ero_ero_diff}.


Again, the resulting behavior indicates greater similarity derived from a detector perfectly matched to the input case. On the other hand, when structural element $B^*$ is generated by dilation of $B$, the histograms derived from $E_{diff}$ and $E^*_{diff}$ will result in similar behavior, as depicted in Figure \ref{fig:hist_ero_ero_albero}.


In this scenario, according to the inclusion logic proposed in \cite{de2017detecting}, the bigger structural element is selected in the following classification phase.
...
\begin{table}[!ht]
\centering
\caption{Training configurations for fixed quality factor}
\label{tab:hist}
\begin{tabular}{|c|c|c|c|}
\hline
\textbf{DATASET}                  & \textbf{SUBSET} & $N_{images}$ & $N_{hist}$ \textbf{per class} \\ \hline
\multirow{2}{*}{UCID}    & 70\%   & 936      & 67932              \\ \cline{2-4} 
& 80\%   & 1070     & 77040              \\ \hline
\multirow{2}{*}{DRESDEN} & 70\%   & 832      & 59904              \\ \cline{2-4} 
& 80\%   & 951      & 98472              \\ \hline
\multirow{2}{*}{RAISE}   & 70\%   & 700      & 50400              \\ \cline{2-4} 
& 80\%   & 800      & 57600              \\ \hline
\end{tabular}
\end{table}
\end{comment}
...
\begin{table}[!ht]
\centering
\caption{Number of classified histograms for each quality factor and filter type}
\label{tab:o_vs_f_hist}
\begin{tabular}{|c|c|c|}
\hline
\textbf{DATASET}                  & \textbf{SUBSET} & $N_{hist}$     	   \\ \hline
\multirow{2}{*}{UCID}    & 70\%   & 28944              \\ \cline{2-3} 
& 80\%   & 19296              \\ \hline
\multirow{2}{*}{DRESDEN} & 70\%   & 25704              \\ \cline{2-3} 
& 80\%   & 17136              \\ \hline
\multirow{2}{*}{RAISE}   & 70\%   & 21600              \\ \cline{2-3} 
& 80\%   & 14400              \\ \hline
\end{tabular}
\end{table}

In Figure \ref{fig:o_vs_f}, cumulative plots for all datasets is presented, depicting accuracy, false positives and false negatives rates for both erosion and dilation detectors.

\begin{figure}[!ht]
\centering
\begin{tabular}{cc}
\centering
\includegraphics[scale=0.35]{o_vs_f_acc_erosion.eps} & \includegraphics[scale=0.35]{o_vs_f_acc_dilation.eps} \\
\includegraphics[scale=0.35]{o_vs_f_fp_erosion.eps} & \includegraphics[scale=0.35]{o_vs_f_fp_dilation.eps} \\
\includegraphics[scale=0.35]{o_vs_f_fn_erosion.eps} & \includegraphics[scale=0.35]{o_vs_f_fn_dilation.eps}
\end{tabular}
\caption{Accuracy (top), false positives (middle) and false negatives (bottom) result plots for erosion (left column) and dilation (right column) detectors when discriminating unprocessed and processed inputs.}
\label{fig:o_vs_f}
\end{figure}
...
\centering
\caption{Number of classified histograms for each quality factor and filter type}
\label{tab:hist_cross}
\begin{tabular}{|c|c|c|}
\hline
\textbf{DATASET} & $N_{images}$ & $N_{hist}$  \\ \hline
UCID & 1338 & 1734048 \\ \hline
DRESDEN & 1189 & 1540944 \\ \hline
RAISE & 1000 & 1296000 \\ \hline
\end{tabular}
\end{table}

Results are presented in terms of accuracy and false positive rate, as we expect to retrieve only negative responses (see Figure \ref{fig:cross_f}).

\begin{figure}[!ht]
\centering
\begin{tabular}{cc}
\centering
\includegraphics[scale=0.35]{cross_acc_erosion.eps} & \includegraphics[scale=0.35]{cross_acc_dilation.eps} \\
\includegraphics[scale=0.35]{cross_fp_erosion.eps} & \includegraphics[scale=0.35]{cross_fp_dilation.eps}
\end{tabular}
\caption{Accuracy (top) and false positives (bottom) result plots for erosion (left column) and dilation (right column) detectors when discriminating correct filtering type.}
\label{fig:cross_f}
\end{figure}
...
\begin{table}[!ht]
\centering
\caption{UCID - Structural Element Accuracy [\%]}
\label{tab:ucid_mask}
\begin{tabular}{c|c|c|c|c|c|c|c|}
\cline{2-8}
\multicolumn{1}{l|}{\textbf{Erosion}}               & \multicolumn{7}{c|}{\textbf{Quality Factor $QF$}}                                                     \\ \hline
\multicolumn{1}{|c|}{\textbf{Mask}}  & \textbf{100} & \textbf{95} & \textbf{90} & \textbf{85} & \textbf{80} & \textbf{75} & \textbf{70} \\ \hline
\multicolumn{1}{|c|}{\textbf{1-15}}  & 100.00          & 99.83       & 99.32      & 98.47       & 97.55       & 96.22       & 94.37       \\ \hline
\multicolumn{1}{|c|}{\textbf{16-25}} & 100.00         & 99.91       & 99.57       & 98.81       & 98.45       & 97.64       & 96.66      \\ \hline
\multicolumn{1}{|c|}{\textbf{26-36}} & 100.00          & 99.93       & 99.14       & 98.50       & 97.58       & 96.40       & 95.06       \\ \hline
\multicolumn{1}{c}{} &\multicolumn{1}{c}{} &\multicolumn{1}{c}{} & \multicolumn{1}{c}{}& \multicolumn{1}{c}{}& \multicolumn{1}{c}{}& \multicolumn{1}{c}{}& \multicolumn{1}{c}{}\\ \cline{2-8}
\multicolumn{1}{l|}{\textbf{Dilation}}               & \multicolumn{7}{c|}{\textbf{Quality Factor $QF$}}                                                     \\ \hline
\multicolumn{1}{|c|}{\textbf{Mask}}  & \textbf{100} & \textbf{95} & \textbf{90} & \textbf{85} & \textbf{80} & \textbf{75} & \textbf{70} \\ \hline
\multicolumn{1}{|c|}{\textbf{1-15}}  & 100.00          & 99.97       & 99.12      & 97.71       & 95.80       & 93.10       & 89.70      \\ \hline
\multicolumn{1}{|c|}{\textbf{16-25}} & 100.00          & 99.96       & 99.33       & 97.80       & 96.87       & 95.34       & 93.30      \\ \hline
\multicolumn{1}{|c|}{\textbf{26-36}} & 100.00          & 99.96       & 98.80       & 96.00      & 94.65       & 91.91       & 87.87       \\ \hline
\end{tabular}
\end{table}
...
\begin{table}[!ht]
\centering
\caption{DRESDEN - Structural Element Accuracy [\%]}
\label{tab:dresden_mask}
\begin{tabular}{c|c|c|c|c|c|c|c|}
\cline{2-8}
\multicolumn{1}{l|}{\textbf{Erosion}}               & \multicolumn{7}{c|}{\textbf{Quality Factor $QF$}}                                                     \\ \hline
\multicolumn{1}{|c|}{\textbf{Mask}}  & \textbf{100} & \textbf{95} & \textbf{90} & \textbf{85} & \textbf{80} & \textbf{75} & \textbf{70} \\ \hline
\multicolumn{1}{|c|}{\textbf{1-15}}  & 100          & 92.58       & 89.17       & 83.96       & 78.25       & 75.04       & 71.58       \\ \hline
\multicolumn{1}{|c|}{\textbf{16-25}} & 100          & 92.25       & 90.05       & 88.44       & 82.25       & 80.92       & 81.37       \\ \hline
\multicolumn{1}{|c|}{\textbf{26-36}} & 100          & 91.29       & 91.02       & 87.06       & 79.65       & 75.95       & 76.36       \\ \hline
\multicolumn{1}{c}{} &\multicolumn{1}{c}{} &\multicolumn{1}{c}{} & \multicolumn{1}{c}{}& \multicolumn{1}{c}{}& \multicolumn{1}{c}{}& \multicolumn{1}{c}{}& \multicolumn{1}{c}{}\\ \cline{2-8}
\multicolumn{1}{l|}{\textbf{Dilation}}               & \multicolumn{7}{c|}{\textbf{Quality Factor $QF$}}                                                     \\ \hline
\multicolumn{1}{|c|}{\textbf{Mask}}  & \textbf{100} & \textbf{95} & \textbf{90} & \textbf{85} & \textbf{80} & \textbf{75} & \textbf{70} \\ \hline
\multicolumn{1}{|c|}{\textbf{1-15}}  & 100          & 92.58       & 89.17       & 83.96       & 78.25       & 75.04       & 71.58       \\ \hline
...
\begin{table}[!ht]
\centering
\caption{RAISE - Structural Element Accuracy [\%]}
\label{tab:raise_mask}
\begin{tabular}{c|c|c|c|c|c|c|c|}
\cline{2-8}
\multicolumn{1}{l|}{\textbf{Erosion}}               & \multicolumn{7}{c|}{\textbf{Quality Factor $QF$}}                                                     \\ \hline
\multicolumn{1}{|c|}{\textbf{Mask}}  & \textbf{100} & \textbf{95} & \textbf{90} & \textbf{85} & \textbf{80} & \textbf{75} & \textbf{70} \\ \hline
\multicolumn{1}{|c|}{\textbf{1-15}}  &  100.00 	& 97.85 	& 90.66 	& 83.30 	& 76.05 	& 71.18 	& 65.35 
\\ \hline
\multicolumn{1}{|c|}{\textbf{16-25}} &  100.00 	& 98.93 	& 91.60 	& 84.65 	& 79.22 &	 76.08 &	 79.46 
\\ \hline
\multicolumn{1}{|c|}{\textbf{26-36}} &  100.00 	& 97.95 	& 92.17 	& 82.91 	& 75.78 	& 70.35 	& 67.78 
\\ \hline
\multicolumn{1}{c}{} &\multicolumn{1}{c}{} &\multicolumn{1}{c}{} & \multicolumn{1}{c}{}& \multicolumn{1}{c}{}& \multicolumn{1}{c}{}& \multicolumn{1}{c}{}& \multicolumn{1}{c}{}\\ \cline{2-8}
\multicolumn{1}{l|}{\textbf{Dilation}}               & \multicolumn{7}{c|}{\textbf{Quality Factor $QF$}}                                                     \\ \hline
\multicolumn{1}{|c|}{\textbf{Mask}}  & \textbf{100} & \textbf{95} & \textbf{90} & \textbf{85} & \textbf{80} & \textbf{75} & \textbf{70} \\ \hline
\multicolumn{1}{|c|}{\textbf{1-15}}  &  100.00 	& 97.99 &	 90.30 & 	 83.55 &	 78.17 	& 75.53 	& 71.09       \\ \hline
\multicolumn{1}{|c|}{\textbf{16-25}} &  100.00 &	 98.75 &	 91.51 &	 85.74 &	 81.53 &	 79.93 	& 82.80 
\\ \hline
\multicolumn{1}{|c|}{\textbf{26-36}} &  100.00 	& 97.58 &	 92.37 &	 83.99 	& 79.13 	& 76.20 	& 74.09 
\\ \hline
\end{tabular}
\end{table}
\end{comment}
